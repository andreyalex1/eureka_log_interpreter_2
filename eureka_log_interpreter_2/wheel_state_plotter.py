...
df1 = pd.read_csv('finished measurements/st_commands.csv',names=headers)
df2 = pd.read_csv('finished measurements/st_states.csv',names=headers)
plt.figure(1)
...
df1 = pd.read_csv('finished measurements/pid_commands.csv',names=headers)
df2 = pd.read_csv('finished measurements/pid_states.csv',names=headers)
plt.figure(2)
time = np.array(pd.to_numeric(df1['time_s'], errors='coerce') + pd.to_numeric(df1['time_ns'], errors='coerce') * 1e-9 )#- df1['time_s'][0])
velocity1 = np.array(pd.to_numeric(df1['velocity1'], errors='coerce'))
plt.plot(time,velocity1)
